...
int main(int argc, char **argv)
{
  ...
  // Init kernel
  typedef BBFMM<17, Barycentric> kernel_type;
  kernel_type K;
  ...
  std::cout << ExpansionTraits<kernel_type>() << std::endl;

  // Init points and charges
  std::vector<source_type> points(M);
  for (int i = 0; i < M; ++i)
    points[i] = source_type(i*2*M_PI/M);

  //std::vector<source_type> points = fmmtl::random_n(M);
  ...
  if (checkErrors) {
    ...
    for (unsigned k = 0; k < result.size(); ++k) {
      ...
      //std::cout << points[k] << "\t" << exact[k] << "\t" << result[k] << "\t" << rel_error << std::endl;
    }
